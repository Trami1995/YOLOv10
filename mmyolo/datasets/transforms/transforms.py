...
#TODO: FilterIgnoreArea
@TRANSFORMS.register_module()
class FilterIgnoreArea(BaseTransform):
    """Filter Ignore Area by processing image"""
    ...
    def transform(self, results):
        ...
        ignore_area = results.get('ignore_area', list())
        if len(ignore_area) == 0: pass

        img = copy.deepcopy(results['img'])

        h, w, _ = img.shape
        ignore_area_mask = np.zeros((h, w))

        for ign in ignore_area:
            x_min = math.floor(min(ign[0::2]))
            x_max = math.ceil(max(ign[0::2]))
            y_min = math.floor(min(ign[1::2]))
            y_max = math.ceil(max(ign[1::2]))
            ignore_area_mask[y_min:y_max+1, x_min:x_max+1] = 1

        ignore_area_mask = ignore_area_mask.astype(dtype=bool)
        img[ignore_area_mask] = [255, 255, 255]
        results['img'] = img
